...
用户在使用社交平台往往会有周期性规律，比如在周末上线的可能要比工作日高，判断`0721`为周日，基于此，增加特征`0714`是否在线作为参考，针对每位指定用户，构造如下特征

| 特征名称                      | 描述                            |
| :-----------------------------: | :-------------------------------: |
| `active_days_in_week`         | 在 07.11–07.17 的活跃天数    |
| `was_active_last_sunday`      | 07.14（上一个星期日）是否在线   |
| `weekday_active_count`        | 周一到周五共活跃几天            |
| `weekend_active_count`        | 周六/周日共活跃几天             |
| `recent_active_gap`           | 距 07.20 最近一次活跃的日期间隔 |
| `avg_daily_interaction_count` | 日均互动行为数                  |
| `weekly_total_views`          | 一周互动行为总和                |
| `daily_active_series`         | 连续活跃分布（如 1100110010）   |
